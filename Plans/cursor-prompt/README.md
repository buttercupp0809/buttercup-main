...
## Engagement, auth, media ops, and R&D batch (34)

| # | File | Delivers | State it targets |
|---|---|---|---|
| 34 | `34-checkin-billing-tabs-email-verify-media-cleanup-llm-rnd.md` | Six features: (A) live LLM-personalized active check-in on chat open, fired on first open and on reopen after a 24h gap, with a static-greeting fallback; (B) billing page Subscription/Passes tabs with token packs hidden behind a flag; (C) email verification on signup via Resend with a hard layout-level gate, Google sign-ups auto-verified; (D) backfill script that keeps the existing hero, drops the 5th persona variant, converts p1-p4 PNG to WebP, uploads to S3, and links `CharacterMedia` in local (now) and prod (gated); (E) Claude Haiku distortion scan that reports first, then a gated hard-delete of flagged pairs (PNG + WebP + rows) from S3 and both DBs on approval; (F) research doc on uncensored companion LLMs the market leaders use; (G) Meta Pixel `Lead` event fired on successful email plus password signup (base pixel + PageView already in `layout.tsx`; `Lead` wired into `SignupForm.tsx`). | Mostly additive/repair on existing flows. Features D and E are prod-touching and split into a safe LOCAL phase plus a human-approved PROD phase (see the file's Guardrail block and Manual steps). |
