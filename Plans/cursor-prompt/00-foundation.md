...
## Goal
Stand up the Poppy monorepo skeleton that every later phase builds on: an npm-workspaces monorepo mirroring Pellow (`frontend/`, `backend/`, `packages/database`, `packages/shared`), the `@poppy/database` Prisma 6 + Postgres + pgvector singleton, shared tooling (TypeScript strict, ESLint, Prettier, Vitest, Playwright), a multistage production Dockerfile, a complete `.env.example`, a root `CLAUDE.md` documenting conventions, and a `.cursor/skills/poppy-design-language/SKILL.md` stub. After this phase `npm install`, `npm run typecheck`, `npm run build`, and `docker build` all succeed locally, and `import { prisma } from "@poppy/database"` resolves from both `frontend/` and `backend/`.
...
```
You are scaffolding "Poppy", a mature-gated AI companion platform. This is Phase 00 (foundation only, no product features).
...
- Prisma singleton: ../Pellow/packages/database/src/client.ts and src/index.ts. The singleton uses globalThis caching + PrismaPg adapter + pool-param injection (connection_limit, pgbouncer=true, connect_timeout, pool_timeout). Export prisma from a package named "@poppy/database" (Pellow uses "@karoli/database"). NEVER call `new PrismaClient()` anywhere except inside packages/database/src/client.ts.
...
- Env catalog: ../Pellow/.env.example structure (adapt vars to Poppy, see Build steps).
...
- packages/shared = shared TypeScript types + Zod schemas, importable as "@poppy/shared".
- packages/database = Prisma 6 + Postgres + pgvector, exported as "@poppy/database".
...
- `package.json`. private root. `"workspaces": ["frontend","backend","packages/*"]`. Scripts (mirror `../Pellow/package.json`, rename to `@poppy/*`):
  - `"typecheck": "(cd packages/database && npx tsc --noEmit) && (cd packages/shared && npx tsc --noEmit) && (cd backend && npx tsc --noEmit)"`
  - `"build": "npm run build --workspaces --if-present"`
  - `"postinstall": "npm run db:generate"`
  - `"db:generate": "npm run generate --workspace=@poppy/database"`
  - `"db:push": "npm run push --workspace=@poppy/database"`
  - `"db:migrate": "npm run migrate --workspace=@poppy/database"`
  - `"db:studio": "npm run studio --workspace=@poppy/database"`
  - `"dev:frontend": "npm run dev --workspace=frontend"`
  - `"dev:backend": "npm run dev --workspace=backend"`
  - `"test": "vitest run"`, `"test:e2e": "playwright test"`, `"lint": "eslint ."`
- `tsconfig.json` (root base), strict true, `target ES2022`, `module NodeNext`, `moduleResolution NodeNext`, `esModuleInterop`, `skipLibCheck`, `resolveJsonModule`, `paths` mapping `@poppy/database` and `@poppy/shared` to their `src`. Each workspace `tsconfig.json` extends this.
...
### 2. `packages/database` (the `@poppy/database` singleton)
- `packages/database/package.json`. name `@poppy/database`, `main: ./dist/index.js`, `types: ./dist/index.d.ts`. Scripts: `generate` (`prisma generate`), `push` (`prisma db push`), `migrate` (`prisma migrate dev`), `studio` (`prisma studio`), `build` (`prisma generate && npx tsc`). Deps: `@prisma/client ^6`, `@prisma/adapter-pg ^6`, `pg ^8`. devDeps: `prisma ^6`, `@types/pg`, `typescript`.
...
### 3. `packages/shared` (the `@poppy/shared` types + Zod schemas)
- `packages/shared/package.json`. name `@poppy/shared`, `main: ./dist/index.js`, `types: ./dist/index.d.ts`, `build: tsc`. Dep: `zod ^3`.
...
- Scaffold with the App Router, React 19, Tailwind 4, TypeScript. `frontend/package.json` depends on `next ^16`, `react ^19`, `react-dom ^19`, `@poppy/database`, `@poppy/shared`, `jose`, `zod`, `tailwindcss ^4`. Script `dev`, `build`, `start`, `lint`.
- `frontend/app/layout.tsx`, `frontend/app/page.tsx`. a placeholder landing page that renders "Poppy" so the dev server and Playwright have a target.
...
- `frontend/lib/db.ts`. `export { prisma } from "@poppy/database";` to prove the singleton import path works from the frontend workspace.
...
- `backend/package.json`. name `@poppy/backend`, `main dist/index.js`. Deps: `@poppy/database`, `@poppy/shared`, `zod`, `ws`. Scripts: `dev` (`tsx watch src/index.ts` or `node --watch` via ts), `build` (`tsc`), `start` (`node dist/index.js`).
- `backend/src/index.ts`. a minimal HTTP server listening on `process.env.PORT ?? 4000` with a `GET /health` route returning `{ ok: true }`. Import `prisma` from `@poppy/database` (do not query yet) to prove the singleton resolves server-side.
- `backend/src/config/flags.ts`. port the `defaultOn(envVar)` pattern from `../Pellow/backend/src/config/flags.ts`. Seed one Poppy flag, e.g. `matureContentEnabled()` reading `MATURE_CONTENT_ENABLED` (default on), so later phases have the pattern.
...
### 7. `.env.example` (Poppy env catalog)
Create `.env.example` mirroring the structure of `../Pellow/.env.example` but with Poppy vars. Group with comments:
- Database: `DATABASE_URL="postgresql://user:password@localhost:5432/poppy?sslmode=disable"`
...
- The Prisma singleton rule, stated explicitly with the canonical import: `import { prisma } from "@poppy/database";`. Never `new PrismaClient()` outside `packages/database/src/client.ts`.
...
### 9. `.cursor/skills/poppy-design-language/SKILL.md` (design system stub)
...
# 5. Vitest smoke (add one trivial test: expect(1+1).toBe(2), plus a test importing { prisma } from "@poppy/database")
npm test
...
- [ ] Workspaces resolve: `import { prisma } from "@poppy/database"` type-checks from BOTH `frontend/lib/db.ts` and `backend/src/index.ts`.
- [ ] `@poppy/shared` imports resolve from frontend and backend.
- [ ] Prisma client generates from the minimal schema; only `packages/database/src/client.ts` constructs `new PrismaClient()`.
- [ ] `docker build -t poppy:local .` succeeds; the final image runs as uid 10001 (non-root) with tini as PID 1.
- [ ] `.env.example` contains every Poppy var group (DB, JWT, OpenRouter/Anthropic/OpenAI, ElevenLabs/Cartesia, Fal/Replicate, Redis, S3/AWS, adult-friendly payment keys, age-vendor keys, Sentry) with no real secrets.
...
- [ ] `.cursor/skills/poppy-design-language/SKILL.md` exists as a stub.
- [ ] No em dashes anywhere in the diff.

## Done criteria
The monorepo installs, type-checks, builds, and containerizes clean locally. The `@poppy/database` singleton is importable from frontend and backend, and no code outside the singleton constructs a Prisma client. `@poppy/shared` is wired. The Dockerfile produces a non-root Node 20 image with ffmpeg + tini + openssl. `.env.example`, `CLAUDE.md`, and the design-skill stub are in place. Phase 01 (auth + age gate) can start against this scaffold.
