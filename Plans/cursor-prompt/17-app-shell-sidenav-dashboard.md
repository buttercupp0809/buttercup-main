...
- Phases 00 to 16 green. In particular:
  - `frontend/app/(protected)/layout.tsx` exists and is **top-nav only** (renders `ButterCupp` label + `<AiDisclosure />`, wraps `requireAgeVerified()`). This is what you rebuild.
...
> Building ButterCupp Phase 17 (dark app shell + left side nav + dashboard + logout + relationship UI). Read `prds/experience-monetization-prd.md` §2.3 and §1 first. This is additive and non-regressive: every existing protected route must stay reachable, the AI-disclosure pill must stay visible on every authenticated surface (SB 243), and the create wizard + settings behavior must be unchanged.
>
> Conventions: Next.js 16 App Router, server components by default, `"use client"` only where interactivity is required. Prisma singleton `import { prisma } from "@buttercupp/database"` (never `new PrismaClient()`). Tailwind 4 with design tokens as CSS vars in `frontend/app/globals.css`. TypeScript strict. Reuse `cn()` from `frontend/lib/utils`. No em dashes anywhere (use commas, periods, parentheses).
>
...
- Accent (rose/skin): `--buttercupp-accent-rose: 344 84% 71%` (approx `#FF6B8A`), soft variant `348 62% 77%` (approx `#E8A0BF`).
- Secondary (violet): `--buttercupp-accent-violet: 262 72% 68%`.
- App dark base: `--buttercupp-app-bg: 240 20% 5%` (approx `#0B0B0F`), raised surface `240 14% 9%`, border `240 10% 18%`, text `0 0% 98%`, muted text `240 6% 65%`.
...
1. **Dark app theme tokens** in `frontend/app/globals.css`
   - Keep the existing `:root` sky tokens for the public/marketing shell. Add a **scoped** dark app theme under a `.buttercupp-app` class (do NOT flip global `:root` to dark, so marketing stays light). Define, on `.buttercupp-app`:
     - `--buttercupp-bg: 240 20% 5%;` `--buttercupp-surface: 240 14% 9%;` `--buttercupp-surface-2: 240 12% 13%;`
     - `--buttercupp-fg: 0 0% 98%;` `--buttercupp-muted: 240 6% 65%;` `--buttercupp-border: 240 10% 18%;`
     - `--buttercupp-accent-rose: 344 84% 71%;` `--buttercupp-accent-rose-soft: 348 62% 77%;` `--buttercupp-accent-violet: 262 72% 68%;`
     - `--buttercupp-primary: var(--buttercupp-accent-rose);` `--buttercupp-primary-fg: 240 20% 5%;`
   - Add a small set of utility helpers (comment them): `.buttercupp-app` sets `background-color: hsl(var(--buttercupp-bg)); color: hsl(var(--buttercupp-fg));`, and a `.buttercupp-scrim` gradient utility (`linear-gradient(to top, hsl(var(--buttercupp-bg)/0.95), transparent)`) reused by cards later.
...
2. **Fonts** in `frontend/app/layout.tsx` (root)
   - Import `Fraunces` and `Geist` (or `Geist_Sans`) from `next/font/google`, expose them as CSS variables `--font-display` and `--font-body` on `<html>` (via `className`). Do not force dark on `<body>`; the app shell opts in via `.buttercupp-app`.
...
4. **Sidebar component** `frontend/components/app-shell/SideNav.tsx` (client)
   - Renders the vertical rail: wordmark `ButterCupp` in `.font-display` at top, then `APP_NAV` links with active-state styling (`usePathname()` to mark the active item with a rose left-accent bar + `aria-current="page"`). Each link has `data-testid={item.testid}` (e.g. `nav-chats`, `nav-discover`, `nav-create`, `nav-settings`).
   ...
   - Collapsible: accept `collapsed` state (icon-only rail) toggled by a button; persist preference in `localStorage` (`buttercupp.sidenav.collapsed`). Collapsed rail still shows tooltips (title attr) and the profile avatar.
...
7. **Rebuild the app shell** `frontend/app/(protected)/layout.tsx` (server)
   ...
   - Structure:
     ...
     ```
   - The `.buttercupp-app` wrapper scopes the dark theme so only in-app surfaces go dark.
...
13. **Regression sweep of protected surfaces**
    - Confirm `/dashboard`, `/gallery`, `/create`, `/settings`, `/chat/[characterId]`, `/chats` all render inside the new shell with the sidebar and the AI-disclosure pill. The create wizard steps and settings actions must be visually reskinned by the dark `.buttercupp-app` wrapper but functionally unchanged.
...
- [ ] The dark cinematic theme applies ONLY inside `.buttercupp-app` (in-app surfaces); the public/marketing shell is unchanged and still light.
