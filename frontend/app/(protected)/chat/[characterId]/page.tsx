...
export default async function ChatPage({
  ...
  // For free users, image messages in history are locked (blurred). We compute
  // the blur server-side so no real s3Key ever reaches the browser.
  const initialMessages = await Promise.all(
    historyRows.reverse().map(async (m) => {
      ...
      const s3Key = m.mediaAsset?.s3Key;

      // Free-tier paywall: image messages owned by a free user are locked.
      // Serve a blurred data URI instead of the real presigned URL so the
      // s3Key never reaches the browser. blurredDataUri has its own documented
      // safe fallback (a valid gradient data URI) so we do NOT wrap it in a
      // catch that could yield undefined (I-3); a locked bubble always has a
      // real blurUri. The character-voiced CTA (m-2) is computed with the SAME
      // deterministic ctaLineFor the live path uses, so reload matches live.
      if (!onPaidPass && s3Key) {
        const blurUri = await blurredDataUri(s3Key);
        return {
          id: m.id,
          role: m.role,
          content: "",
          ...
          blurUri,
          ctaText: ctaLineFor(m.id, character.name),
        };
      }
      ...
      return {
        ...
        imageUrl: s3Key ? signAssetUrl(s3Key) : undefined,
      };
    }),
  );
